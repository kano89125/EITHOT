...
namespace inplace { 
	...
	int merge_seq_perm_in_dim(int* unmerged_dim, int* merged_dim, int* permutation, int unmerged_rank) {
		...
		merged_index = merged_rank;
		for(; merged_index < unmerged_rank; ++merged_index) { merged_dim[merged_index] = -1;}
	...
	void merge_matched_dim_pre_order(int *origin_dim, int *unmatched_dim, int init_unmatched_index, int rank) {
		if(init_unmatched_index != 0) {
			unmatched_dim[0] = 1;
			for(int i = 0; i < init_unmatched_index; ++i) { unmatched_dim[0] *= origin_dim[i];}
			std::copy(origin_dim + init_unmatched_index, origin_dim + rank, unmatched_dim + 1);
		} else { std::copy(origin_dim, origin_dim + rank, unmatched_dim);}
	}

	void merge_matched_dim_post_order(int *origin_dim, int *unmatched_dim, int init_unmatched_index, int rank) {
		int unmatched_rank;
		if(init_unmatched_index < rank - 2) {
			unmatched_rank = init_unmatched_index + 2;
			unmatched_dim[unmatched_rank - 1] = 1;
			for(int i = init_unmatched_index + 1; i < rank; ++i) { unmatched_dim[unmatched_rank - 1] *= origin_dim[i];}
			std::copy(origin_dim, origin_dim + init_unmatched_index + 1, unmatched_dim);
		} else { 
			unmatched_rank = rank;
			std::copy(origin_dim, origin_dim + rank, unmatched_dim);
		}
	}

	void merge_matched_perm_pre_order(int *round_trans_perm, int *trans_unmatched_perm, int init_unmatched_index, int rank) {
		if(init_unmatched_index != 0) {
			trans_unmatched_perm[0] = 0; // minmum of perm
			...
			rerank_permutation(trans_unmatched_perm, unmatched_rank);
	...
	int *pre_low_order_dim, int *pre_low_order_perm, int *pre_round_trans_perm, int rank) {
		...
		int unmatched_rank;
		if(init_unmatched_index != 0) { unmatched_rank = rank - init_unmatched_index + 1;}
		else { unmatched_rank = rank;}
		...
		int *unmatched_dim = new int[unmatched_rank];
		merge_matched_dim_pre_order(tensor_dim_reordered, unmatched_dim, init_unmatched_index, rank);
				
		pre_round_trans_perm_generation(transpose_permutation, cur_tensor_indices, pre_round_trans_perm, rank, init_unmatched_index);
		
		int *trans_unmatched_perm = new int[unmatched_rank];
		merge_matched_perm_pre_order(pre_round_trans_perm, trans_unmatched_perm, init_unmatched_index, rank);
			
		int catanzaro_rank = merge_seq_perm_in_dim(unmatched_dim, pre_low_order_dim, trans_unmatched_perm, unmatched_rank);
			
		merge_seq_perm(trans_unmatched_perm, pre_low_order_perm, unmatched_rank);
			
		delete [] tensor_dim_reordered;
		delete [] unmatched_dim;
		delete [] trans_unmatched_perm;
	...
	int post_order_trans_perm_generation(std::vector<int> cur_tensor_indices, int *tensor_dim, int *transpose_permutation,
	 int *post_low_order_dim, int *post_low_order_perm, int *post_round_trans_perm, int rank) {
		...
		int unmatched_rank;
		if(init_unmatched_index < rank - 2) { unmatched_rank = init_unmatched_index + 2;}
		else { unmatched_rank = rank;}
		...
		int *unmatched_dim = new int[unmatched_rank];
		merge_matched_dim_post_order(tensor_dim_reordered, unmatched_dim, init_unmatched_index, rank);
		...
		int *trans_unmatched_perm = new int[unmatched_rank];
		merge_matched_perm_post_order(post_round_trans_perm, trans_unmatched_perm, init_unmatched_index, rank);

		int catanzaro_rank = merge_seq_perm_in_dim(unmatched_dim, post_low_order_dim, trans_unmatched_perm, unmatched_rank);

		merge_seq_perm(trans_unmatched_perm, post_low_order_perm, unmatched_rank);
