...
namespace _Nd {
	...
	void Series_Low_order_transpose(T *data, int source, int *tensor_dim, int *transpose_permutation, int rank, int tensor_vol, double ALPHA) {
		...
		std::vector<int> target_perm_vec(rank, 0);
		target_perm_vec.assign(transpose_permutation, transpose_permutation + rank);
		if(cur_tensor_indices == target_perm_vec) { return;}
	...
	void transpose(T* data, int source, void* dim, void* permutation, int rank, int num_block, double ALPHA) {
		...
		int padding_dim_pos = -1, decompose_dim_pos = -1, origin_dim_size, padding_dim_size, origin_vol = vol;
		int max_dim_pos = find_proper_max_dim_pos(tensor_dim, rank);
		if(tensor_dim[max_dim_pos] > LARGE_SIZE && num_block > 1) {
			origin_dim_size = tensor_dim[max_dim_pos];
			decompose_dim_pos = max_dim_pos;
			if (tensor_dim[max_dim_pos] % num_block != 0) {
		...
		if(padding_dim_pos != -1) {
			...
			vol = vol / origin_dim_size * padding_dim_size;
		}	
		...
		} else { Series_Low_order_transpose(data, source, tensor_dim, transpose_permutation, rank, vol, ALPHA);}
		if(padding_dim_pos != -1) {
			vol = origin_vol;
			tensor_dim[padding_dim_pos] = origin_dim_size;
